...
import 'package:cloud_firestore/cloud_firestore.dart'; // Firestore for getting data
...
class LineChartPage extends StatelessWidget {
        ...
        child: FutureBuilder<List<FlSpot>>(
          ...
          builder: (context, snapshot) {
            ...
            if (!snapshot.hasData || snapshot.data!.isEmpty) {
              return Center(child: Text("No data available for the selected habit/timeframe"));
            }
            return LineChart(
              LineChartData(
                ...
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(
                      show: true,
                      color: Colors.blue.withOpacity(0.2),  // Replace `colors` with `color`
                    ),
                  ),
                ],
              ),
  ...
  Future<List<FlSpot>> _getChartData(String timeframe, String habit) async {
    ...
    if (habit == 'All') {
      rawData = await _getAllHabitData(timeframe);  // Aggregate data for all habits
    } else {
  ...
  Widget _getBottomTitles(double value, TitleMeta meta) {
    ...
    switch (selectedTimeframe) {
      ...
      // Map X-axis values to months of the year
        text = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][value.toInt()];
        break;
      default:
        text = '';
    }
  ...
  Widget _getLeftTitles(double value, TitleMeta meta) {
    ...
    return SideTitleWidget(
      child: Text(value.toInt().toString(), style: style),  // Show integer values for the Y-axis
